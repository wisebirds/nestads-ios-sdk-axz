...
SWIFT_CLASS_NAMED("NestAdsPartnerBridgeSupport")
@interface NestAdsPartnerBridgeSupport : NSObject
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end

/// 광고 수명 핸들. RTB: 메인 PartnerAdTracker 가 strong 보유(teardown 시 destroy).
/// mediation: 메인 미보유 — 파트너 세션이 로드 완료 시 자체 해제 책임.
SWIFT_PROTOCOL_NAMED("NestAdsPartnerLifecycleBridge")
@protocol NestAdsPartnerLifecycleBridge
- (void)resume;
- (void)pause;
- (void)destroy;
@end

/// mediation 워터폴 위임 (COVI · AdFit).
/// units: NestAdsPartnerUnit.toDictionary() 출력 = 서버 wire(snake_case) 스키마 그대로.
/// options: targeting(age/gender/idfa) · display(cornerRadius) · environment(“prod”/“dev”).
/// completion: 성공 payload { adView: UIView, lifecycle?: LifecycleBridge, sizing?: dict } / 실패 NSError
/// (domain “com.nestads.partner.bridge”, code 1=noFill · 2=sdkError, userInfo[“adNetworkKey”]=실패 유닛 키).
/// 스레드 무보장 — 호출측(메인)이 main hop.
/// <em>completion 은 정확히 1회 호출한다.</em> 양 레포 모두 중복 호출 방어가 없어(선재) 두 번 부르면
/// 매체 delegate 가 이중 통지된다. 파트너는 once-guard 로 단일 관문을 유지할 것.
/// <em>sizing 키 스키마</em> — 키 <em>존재</em>로 판별하며 두 형태는 <em>상호 배타</em>다. 단위는 모두
/// <em>pt(= Android dp)</em> 이지 픽셀이 아니다(<code>/ UIScreen.main.scale</code> 변환 금지 — @2x 에서 반토막).
/// <ul>
///   <li>
///     이미지형(AdFit): <code>{ "width": Int, "height": Int }</code>  — 소재 원본 크기
///   </li>
///   <li>
///     동영상형(Covi):  <code>{ "heightRatio": Double }</code>        — 실제 렌더에 쓴 height/width
///     둘 다 오면 수신측은 width/height 를 우선한다(계약 위반 방어). 값 위반(0·음수·비유한)은
///     힌트 부재로 강등되어 호출측 기존 폴백(units 기반 계산)을 탄다.
///   </li>
/// </ul>
SWIFT_PROTOCOL_NAMED("NestAdsPartnerMediationBridge")
@protocol NestAdsPartnerMediationBridge
- (void)nestads_handleMediationAd:(NSArray<NSDictionary<NSString *, id> *> * _Nonnull)units options:(NSDictionary<NSString *, id> * _Nonnull)options container:(UIView * _Nonnull)container completion:(void (^ _Nonnull)(NSDictionary * _Nullable, NSError * _Nullable))completion;
@end

/// 재생 제어. 파트너 adView(자사 클래스만 — 서드파티 @objc 소급 conform 금지)가 conform,
/// 메인이 as? 캐스팅 후 main-thread 프록시로 래핑.
SWIFT_PROTOCOL_NAMED("NestAdsPartnerPlaybackControlBridge")
@protocol NestAdsPartnerPlaybackControlBridge
- (void)play;
- (void)pause;
- (void)replay;
- (void)mute;
...
/// RTB 위임 (Moloco) — 초기화 · bid token · 렌더.
/// configs: [{ “partner”: String, “config”: [String: String] }]
/// creative: { “adm”: String, “adUnitId”: String, “adNetworkKey”: String, “width”?: Int, “height”?: Int }
/// — <code>adUnitId</code> 는 배너 생성 입력, <code>adNetworkKey</code> 는 파트너 쪽 어댑터 라우팅 가드용으로 <em>둘 다 필수 운반</em>.
/// 빠지면 파트너가 어댑터를 못 골라 전 RTB 가 “[9999] Unknown ad network” 로 거절된다.
/// render 의 completion 도 <em>정확히 1회</em> 호출한다(위 mediation 과 동일 계약).
SWIFT_PROTOCOL_NAMED("NestAdsPartnerRtbBridge")
@protocol NestAdsPartnerRtbBridge
- (void)nestads_initialize:(NSArray<NSDictionary<NSString *, id> *> * _Nonnull)configs;
- (void)nestads_bidTokenWithCompletion:(void (^ _Nonnull)(NSDictionary<NSString *, NSString *> * _Nonnull, NSDictionary<NSString *, NSString *> * _Nonnull))completion;
- (void)nestads_render:(NSDictionary<NSString *, id> * _Nonnull)creative options:(NSDictionary<NSString *, id> * _Nonnull)options container:(UIView * _Nonnull)container didClick:(void (^ _Nonnull)(void))didClick completion:(void (^ _Nonnull)(NSDictionary * _Nullable, NSError * _Nullable))completion;
@end
